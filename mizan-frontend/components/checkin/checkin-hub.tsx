...
type CheckinHubProps = {
  onMorning: () => void;
  onEvening: () => void;
  compact?: boolean;
};

export function CheckinHub({ onMorning, onEvening, compact }: CheckinHubProps) {
  ...
  return (
    <div className={compact ? "" : "page-enter"}>
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-primary to-blue-400 bg-clip-text text-transparent pb-1">
          Daily Check-ins
        </h2>
        <p className="text-sm text-on-surface-variant mt-2 max-w-2xl mx-auto">
          Morning and evening rituals to track mood, sleep, and your day.
        </p>
      </div>

      {loading && <p className="text-sm text-on-surface-variant text-center">Loading check-in status...</p>}
      {error && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600 max-w-md mx-auto mb-4">
          {error}
        </div>
      )}
        ...
        <Card className="shadow-sanctuary hover:shadow-sanctuary-lg transition-all duration-300">
          <CardContent className="p-6 space-y-4 text-center">
            <div className="flex justify-center items-center gap-3">
              ...
              <h3 className="text-xl font-bold">Morning Check-in</h3>
            </div>
            ...
            <Button size="lg" className="w-full h-12" onClick={onMorning} disabled={!morningState.canStart}>
        ...
        <Card className="shadow-sanctuary hover:shadow-sanctuary-lg transition-all duration-300">
          <CardContent className="p-6 space-y-4 text-center">
            <div className="flex justify-center items-center gap-3">
              ...
              <h3 className="text-xl font-bold">Evening Check-in</h3>
            </div>
            ...
            <Button size="lg" className="w-full h-12" onClick={onEvening} disabled={!eveningState.canStart}>
