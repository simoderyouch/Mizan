...
const TYPE_CONFIG: Record<ResourceType, { icon: React.ElementType; label: string; color: string }> = {
  VIDEO: { icon: Video, label: "Video", color: "bg-indigo-50 text-indigo-600" },
  ARTICLE: { icon: FileText, label: "Article", color: "bg-emerald-50 text-emerald-600" },
  EXERCISE: { icon: Dumbbell, label: "Exercise", color: "bg-amber-50 text-amber-600" },
};
...
export default function ResourcesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Unable to load resources."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page-enter space-y-8 max-w-4xl mx-auto px-1">
      <div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">Wellbeing resources</h1>
        <p className="text-on-surface-variant mt-2">
          Personalized resources based on your current mood.
        </p>
      </div>
      ...
      {currentList.length === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            <p className="text-on-surface-variant">No resources available right now.</p>
          </CardContent>
        </Card>
      ) : (
