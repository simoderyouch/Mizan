...
export default function GoalDetailPage() {
  ...
  const fetchGoal = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Unable to load this goal."));
    } finally {
      setLoading(false);
    }
  }, [goalId]);
  ...
  const logProgress = async (e: React.FormEvent) => {
    ...
    if (!Number.isFinite(parsedValue) || parsedValue <= 0) {
      toast({ title: "Invalid value", description: "Progress must be greater than 0.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast({ title: "Error", description: getApiErrorMessage(err, "Unable to save progress."), variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const deactivate = async () => {
    try {
      ...
      toast({ title: "Goal deactivated", description: "This goal was removed from your active goals." });
      router.push("/goals");
    } catch (err: unknown) {
      toast({ title: "Error", description: getApiErrorMessage(err, "Unable to deactivate this goal."), variant: "destructive" });
    }
  };
  ...
  if (!goal) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/goals"><Button variant="secondary" className="mt-4">Back</Button></Link>
      </div>
    );
  }
    ...
    <div className="page-enter max-w-2xl mx-auto space-y-6 px-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="sanctuary-card-subtle text-center !p-4">
          <span className="label-sanctuary">Today</span>
        ...
        <div className="sanctuary-card-subtle text-center !p-4">
          <span className="label-sanctuary">Target</span>
      ...
      <Card>
        <CardContent>
          <h3 className="font-bold mb-4">Log progress</h3>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Deactivate this goal?</DialogTitle>
            <DialogDescription>
              This will remove the goal from active goals. History will be kept.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
