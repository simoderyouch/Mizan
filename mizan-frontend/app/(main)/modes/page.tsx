...
export default function ModesPage() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Unable to load mode stats."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const startMode = async (mode: Mode) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Unable to start this mode."));
    } finally {
  ...
  return (
    <div className="page-enter space-y-8 max-w-4xl mx-auto px-1">
      <div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">Work modes</h1>
        <p className="text-on-surface-variant mt-2">
          Select your current mode and track your productivity.
        </p>
      </div>
          ...
          return (
            ...
            >
              <CardContent className="text-center">
                ...
                {todayMin > 0 && (
                  <p className="text-xs text-on-surface-variant">{formatMinutes(todayMin)} today</p>
                )}
                {isActive && (
