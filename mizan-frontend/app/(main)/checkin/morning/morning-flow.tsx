...
export type MorningCheckinFlowProps = {
  embedded?: boolean;
  onBack?: () => void;
  onOpenVoice?: () => void;
  onComplete?: () => void;
...
export function MorningCheckinFlow({
  embedded = false,
  onBack,
  onOpenVoice,
  onComplete,
}: MorningCheckinFlowProps = {}) {
  ...
  const openVoice = () => {
    if (embedded && onOpenVoice) {
      onOpenVoice();
      return;
    }
    router.push("/checkin/voice?period=MORNING&return=/checkin/morning");
  };
  ...
  return (
    <div className={`page-enter max-w-2xl mx-auto px-1 ${embedded ? "pb-4" : "pb-16"}`}>
      <button
        type="button"
        onClick={goBack}
        className="inline-flex items-center text-sm text-on-surface-variant hover:text-primary mb-6 transition-colors"
      >
      ...
      {step === "mode" && (
        <div className="grid sm:grid-cols-2 gap-4 page-enter">
          <Card className="cursor-pointer hover:border-primary/50 transition-all hover:shadow-sanctuary group" onClick={openVoice}>
      ...
      {step === "result" && report && (
        <div className="page-enter space-y-8">
          ...
          <Button onClick={finish} className="w-full h-14 text-lg rounded-xl shadow-md">
            Go to Dashboard
          </Button>
        </div>
      )}
