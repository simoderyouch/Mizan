...
import { VOICE_TIMING, voiceTimingLabel } from "@/lib/voiceTiming";
import { startWebSilenceMonitor } from "@/lib/voiceSilenceMonitor";
...
const HANDS_FREE = true;

export type VoiceCheckinFlowProps = {
  embedded?: boolean;
...
export function VoiceCheckinFlow({
  embedded = false,
  period: periodProp,
  onBack,
  onComplete,
}: VoiceCheckinFlowProps = {}) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const requestedPeriod = periodProp ?? searchParams.get("period");
  ...
  const setQuestionPlaying = (playing: boolean) => {
  ...
  const scheduleAutoRecord = () => {
    if (!HANDS_FREE || step !== "session" || !session) return;
    if (isRecording || processingAudio || submitting || isQuestionPlayingRef.current) return;
    if (transcriptions.some((entry) => entry.question_index === currentQ)) return;
    window.setTimeout(() => {
      if (isQuestionPlayingRef.current || isRecording || processingAudio || submitting) return;
  ...
  const playQuestionAudio = (audio: HTMLAudioElement): Promise<void> =>
    new Promise((resolve) => {
      let settled = false;
      const done = () => {
        if (settled) return;
      ...
      const fallbackTimer = window.setTimeout(done, VOICE_TIMING.questionAudioFallbackMs);
  ...
  const transcribeRecording = async (audioBlob: Blob, questionIndex: number) => {
    ...
    try {
      ...
      setTranscriptions((prev) => {
        ...
        const next = [...withoutQuestion, { question_index: questionIndex, question_id: questionId, transcription: text }];
  ...
  const startRecording = async () => {
    if (!session || processingAudio || submitting || isQuestionPlayingRef.current) return;
    stopQuestionPrompt();
    ...
    try {
      ...
      stopSilenceMonitor();
      if (HANDS_FREE) {
        silenceMonitorRef.current = startWebSilenceMonitor(stream, () => stopRecordingRef.current());
      }
  ...
  return (
    <div className={`page-enter max-w-2xl mx-auto px-1 ${embedded ? "pb-4" : ""}`}>
      <button
        type="button"
        onClick={goBack}
        className="inline-flex items-center text-sm text-on-surface-variant hover:text-primary mb-6"
      >
      ...
      {step === "start" && (
        <div className="text-center py-12 page-enter">
          ...
          <p className="text-xs text-on-surface-variant/80 mt-2 max-w-sm mx-auto">{voiceTimingLabel()}</p>
        </div>
      )}
      ...
      {step === "session" && session && (
        <div className="page-enter">
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-on-surface-variant mt-3">{voiceTimingLabel()}</p>
          </div>

          <div className="flex flex-col items-center gap-6">
            <button
              onClick={toggleRecording}
              disabled={processingAudio || submitting || isQuestionPlaying}
            ...
            {!HANDS_FREE ? (
              <Button onClick={toggleRecording} disabled={processingAudio || submitting || isQuestionPlaying} className="w-full">
                {isRecording ? "Stop my answer" : "Answer this question"}
              </Button>
            ) : isRecording ? (
              <Button onClick={stopRecording} disabled={processingAudio || submitting || isQuestionPlaying} variant="secondary" className="w-full">
                Stop early
              </Button>
            ) : null}
