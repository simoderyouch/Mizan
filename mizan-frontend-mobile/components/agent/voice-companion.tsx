...
export function VoiceCompanion() {
  ...
  const getMicrophoneErrorMessage = (err?: unknown) => {
    if (typeof navigator === "undefined" || !navigator.mediaDevices?.getUserMedia) {
      return "Microphone APIs are unavailable on this device. Open the app in HTTPS and try again.";
    }
    const name = typeof err === "object" && err && "name" in err ? String((err as { name?: unknown }).name ?? "") : "";
    if (name === "NotAllowedError" || name === "SecurityError") {
      if (typeof window !== "undefined" && !window.isSecureContext) {
        return "Microphone is blocked by browser security. Open the app in HTTPS, accept the certificate, then retry.";
      }
      return "Microphone permission denied. Allow microphone access in your browser settings and retry.";
    }
    if (name === "NotFoundError" || name === "DevicesNotFoundError") {
      return "No microphone detected on this device.";
    }
    if (name === "NotReadableError" || name === "TrackStartError") {
      return "Microphone is busy in another app. Close other apps and retry.";
    }
      ...
      if (chatRes.agent_audio_base64) {
        ...
        try {
          ...
          toast({
            ...
            description: "Tap Answer again to resume playback.",
          });
        }
      }
    } catch (err) {
  ...
  const startRecorderCapture = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: getMicrophoneErrorMessage(err),
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="relative flex flex-col items-center justify-start pt-2 pb-8 px-8 space-y-8 w-full max-w-2xl mx-auto">
      ...
      <div className="relative flex items-center justify-center h-40 w-40">
        ...
        <div
          className={cn(
            "absolute inset-4 rounded-full blur-xl transition-all duration-1000 ease-in-out",
            isRecording ? "bg-red-500/10 scale-110" :
              isPlaying ? "bg-primary/20 scale-105" :
                isProcessing ? "bg-blue-400/15 scale-100" :
          ...
          <>
            {captureMode === "record" ? (
              ...
              >
                ...
                {isRecording ? "Stop & send" : "Answer"}
              </button>
            ) : (
              <button
                onClick={() => void sendCurrentTranscript()}
                disabled={isProcessing || !draftTranscript.trim()}
