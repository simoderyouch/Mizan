...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 left-0 right-0 z-50 bg-surface/95 backdrop-blur-xl border-b border-outline-variant/10">
      <div className="px-4 h-14 flex items-center justify-between">
        <Link href="/dashboard" className="inline-flex items-center active:scale-95 transition-transform duration-200 select-none">
          <Image
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu open={isNotificationsOpen} onOpenChange={setIsNotificationsOpen}>
            <DropdownMenuTrigger asChild>
              <button className="relative rounded-full p-2 text-on-surface-variant hover:bg-surface-container transition-colors">
            ...
            <DropdownMenuContent align="end" className="w-[22rem] max-w-[92vw] p-0">
                ...
                {isLoadingNotifications ? (
                  ...
                  notifications.map((item) => (
                    <button
                      ...
                      className={cn(
                        "mb-2 w-full rounded-lg border p-3 text-left transition-all duration-200",
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="rounded-full overflow-hidden active:scale-95 transition-transform duration-200 select-none">
                <Avatar className="h-8 w-8 ring-2 ring-transparent active:ring-primary/20 transition-all">
                  {student?.photo_url ? (
                    <AvatarImage src={student.photo_url} alt="avatar" className="object-cover" />
                  ) : null}
                  <AvatarFallback className="text-[10px] font-bold bg-surface-container-high text-on-surface">
