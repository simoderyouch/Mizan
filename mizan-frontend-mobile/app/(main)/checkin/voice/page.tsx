...
export default function VoiceCheckinPage() {
  ...
  const getMicrophoneErrorMessage = (err?: unknown) => {
    if (typeof window === "undefined" || !navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
      return "L'enregistrement audio n'est pas supporté sur cet appareil. Utilisez HTTPS et un navigateur récent.";
    }
    const name = typeof err === "object" && err && "name" in err ? String((err as { name?: unknown }).name ?? "") : "";
    if (name === "NotAllowedError" || name === "SecurityError") {
      if (!window.isSecureContext) {
        return "Le navigateur bloque le micro pour des raisons de sécurité. Ouvrez l'app en HTTPS, acceptez le certificat puis réessayez.";
      }
    ...
    return "Accès au microphone indisponible.";
  };
  ...
  const startSession = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Impossible de démarrer la session vocale."));
    } finally {
      setStarting(false);
    }
  };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        cleanupRecorder();
        if (blob.size === 0) {
          setError("Aucun son détecté. Réessayez en parlant plus près du micro.");
          return;
        }
  ...
  const submitSession = async () => {
    ...
    try {
      ...
      autoSubmittedSessionRef.current = null;
      setError(getApiErrorMessage(err, "Erreur lors de l'analyse vocale."));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const createTasksFromRecommendations = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Impossible de créer des tâches depuis les recommandations vocales."));
    } finally {
      ...
      {step === "start" && (
        <div className="text-center py-12 page-enter">
          <Badge className="mb-4">
            ...
            {period === "MORNING" ? "Check-in vocal du matin" : "Check-in vocal du soir"}
          </Badge>
          <h1 className="text-2xl sm:text-3xl font-bold text-primary mb-2">
            {period === "MORNING" ? "Je vous écoute pour votre matinée" : "Je vous écoute pour votre soirée"}
          </h1>
          <p className="text-on-surface-variant mb-8">
            {period === "MORNING"
              ? "Répondez aux questions personnalisées sur votre nuit et votre état du jour."
              : "Répondez aux questions personnalisées sur votre journée et votre état du soir."}
          </p>
          ...
          <p className="text-sm text-on-surface-variant mt-6">Appuyez pour commencer</p>
        </div>
      )}
        ...
        <div className="page-enter">
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <Button onClick={toggleRecording} disabled={processingAudio || submitting} className="w-full">
              {isRecording ? "Arrêter ma réponse" : "Répondre à cette question"}
            </Button>
            ...
            >
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Analyser mes réponses"}
            </Button>
          </div>
        </div>
      )}
      ...
      {step === "result" && analysis && (
        <Card className="page-enter">
          <CardContent className="text-center">
            ...
            <h2 className="text-xl font-bold mb-4">Analyse terminée</h2>
            ...
            {analysis.recommendations.length > 0 && (
              <div className="text-left mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                  <h3 className="font-bold">{period === "MORNING" ? "Propositions de tâches" : "Recommandations"}</h3>
                  {period === "MORNING" && (
                    <Badge variant="secondary">{selectedRecommendationCount}/{analysis.recommendations.length} sélectionnée(s)</Badge>
                  )}
                </div>
                {period === "MORNING" && (
