...
export default function HistoryPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Impossible de charger l'historique."));
    } finally {
      setLoading(false);
    }
  }, [days]);
  ...
  return (
    <div className="page-enter space-y-6 w-full mx-auto pb-4">
      <div>
        <span className="label-sanctuary text-primary">Analyse de l&apos;humeur</span>
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mt-1">Votre Voyage Intérieur</h1>
        <p className="text-on-surface-variant mt-2 max-w-xl">
          Explorez vos tendances émotionnelles et découvrez les patterns de votre bien-être.
        </p>
      </div>
      ...
      <div className="flex justify-start sm:justify-end">
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            <span className="label-sanctuary">Score bien-être</span>
        ...
        <Card className="lg:col-span-2 shadow-sanctuary-sm">
          <CardContent className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
              <h3 className="text-lg sm:text-xl font-bold">Intensité Émotionnelle</h3>
                ...
                <LineChart data={moodData.map((p) => ({ ...p, dateLabel: formatDateShort(p.date) }))}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number, name: string) =>
                      name === "mood_score" ? [`${v}/5`, "Humeur"] : [`${v}h`, "Sommeil"]
                    }
                  />
        ...
        <div className="space-y-4">
          <div className="sanctuary-card-subtle bg-surface-container-lowest">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="font-bold text-sm">Sommeil</h4>
            </div>
            <p className="text-xs text-on-surface-variant">
              Moyenne de <strong className="text-primary">{avgSleep}h</strong> cette période.
            </p>
          </div>
          <div className="sanctuary-card-subtle bg-surface-container-lowest">
            <div className="flex items-center gap-2 mb-2">
              <BookOpen className="h-4 w-4 text-primary" />
              <h4 className="font-bold text-sm">Études</h4>
            </div>
            <p className="text-xs text-on-surface-variant">
              Suivez l&apos;impact de vos sessions sur votre humeur.
            </p>
          </div>
          <div className="sanctuary-card-subtle bg-surface-container-lowest">
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="h-4 w-4 text-primary" />
              <h4 className="font-bold text-sm">Méditation</h4>
            </div>
            <p className="text-xs text-on-surface-variant">
              Les pauses actives améliorent votre concentration.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="shadow-sanctuary-sm">
            <CardContent className="p-4 sm:p-6 h-full">
              <h3 className="text-xl font-bold mb-6">Palette Émotionnelle</h3>
          ...
          <Card className="shadow-sanctuary-sm">
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold">Rapport hebdo</h3>
                <Link href="/history/weekly">
                  <Button variant="ghost" size="sm" className="px-2 sm:px-3 text-primary">Tout voir</Button>
                </Link>
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-on-surface">
                          {item.type === "morning" ? "Check-in Matinal" : "Bilan du Soir"}
                        </h3>
