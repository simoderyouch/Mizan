...
export default function WeeklyReportPage() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Impossible de charger le rapport hebdomadaire."));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!report) {
    return (
      <div className="page-enter max-w-2xl mx-auto text-center py-12">
        <p className="text-on-surface-variant mb-4">Aucun rapport disponible cette semaine.</p>
        <Link href="/history"><Button variant="secondary">Retour</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="page-enter max-w-3xl mx-auto space-y-6 px-1">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold">Rapport Hebdomadaire</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="sanctuary-card-subtle text-center !p-4">
          <span className="label-sanctuary">Humeur moy.</span>
          <p className="text-2xl font-bold text-primary mt-1">{report.avg_mood.toFixed(1)}</p>
        </div>
        <div className="sanctuary-card-subtle text-center !p-4">
          <span className="label-sanctuary">Sommeil moy.</span>
        ...
        <div className="sanctuary-card-subtle text-center !p-4">
          <span className="label-sanctuary">Objectifs</span>
      ...
      <Card>
        <CardContent>
          <h3 className="font-bold mb-3">Niveau de stress</h3>
          <Badge variant={
            report.stress_level === "LOW" ? "success" :
            report.stress_level === "MEDIUM" ? "warning" : "destructive"
          }>
            {report.stress_level === "LOW" ? "Bas — Serein" :
             report.stress_level === "MEDIUM" ? "Modéré — En équilibre" :
              "Élevé — Prenez soin de vous"}
          </Badge>
        </CardContent>
      </Card>
      ...
      {report.mode_distribution.length > 0 && (
        <Card>
          <CardContent>
            <h3 className="font-bold mb-4">Répartition des modes</h3>
