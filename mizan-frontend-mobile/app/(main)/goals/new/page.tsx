...
export default function NewGoalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!Number.isFinite(parsedTarget) || parsedTarget <= 0) {
      setError("La valeur cible doit être supérieure à 0.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="page-enter max-w-lg mx-auto px-1">
      ...
      <h1 className="text-2xl font-bold mb-6">Nouvel objectif</h1>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="title"
                placeholder="Ex: Réussir mon semestre"
                value={title}
            ...
            <Button type="submit" className="w-full" disabled={loading}>
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Créer l'objectif"}
            </Button>
          </form>
        </CardContent>
      </Card>
