...
export function MorningCheckinScreen({ navigation }: { navigation: Nav }) {
  ...
  const submit = async () => {
    ...
    try {
      const response = await checkinsApi.createMorning({
        ...
        sleep_hours: extractMetricFromAnswers(questions, answers, "sleep_hours") as number | undefined,
      });
      setResult(response.executive_summary ?? "Check-in saved.");
      setPlanTasks(response.detailed_action_plan ?? []);
      setTasksCreated("");
      setStep("result");
    } catch (err) {
      setError(getApiErrorMessage(err, "Could not save check-in."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const createPlanTasks = async (selected: string[]) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Could not create plan tasks."));
    } finally {
    ...
    <Screen variant="stackBare">
      ...
      {step === "format" ? (
        <RitualFormatPicker
          ...
          onVoice={() => navigation.navigate("VoiceCheckin", { period: "MORNING" })}
