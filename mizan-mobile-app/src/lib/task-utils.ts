...
export type SourceFilter = "all" | "manual" | "ai";

export function filterBySource(tasks: Task[], sourceFilter: SourceFilter) {
  if (sourceFilter === "all") return tasks;
  if (sourceFilter === "manual") return tasks.filter(isManualTask);
  return tasks.filter(isAiTask);
}
...
export function taskStats(tasks: Task[], today = toIsoToday()) {
  const grouped = groupTasks(tasks, today);
  const aiOpen = tasks.filter((t) => isAiTask(t) && isTaskPending(t)).length;
  return {
    todayPending: grouped.today.filter(isTaskPending).length,
